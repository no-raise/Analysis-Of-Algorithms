...
if __name__=="__main__":
    ...
    insertion_time = [0.0, 0.36, 14.83, 1476.58, 5664.06]
    Table = PrettyTable(["Input Size", "Insertion Sort", "Merge Sort", "Heap Sort"])
    for i, size in enumerate(input_sizes):
        array = create_array(size, bound)
        array_copy = list(array)

        # Calculations for heap sort
        start_time = time.process_time()
        heap_sort(array)
        end_time = time.process_time()
        time_heap_sort = end_time - start_time
        ...
        # Calculations for merge sort
        start_time = time.process_time()
        merge_sort(array_copy)
        end_time = time.process_time()
        time_merge_sort = end_time - start_time

        Table.add_row([size, round(time_insertion_sort, 2), round(time_merge_sort, 2), round(time_heap_sort, 2)])
